...
# + Duplicate GTF gene_name masking (prevents repeated labels)
# + VolcanoCompare (strict) + 6 category heatmaps
# + GO bipartite network / term projection
# ============================
...
# ---- Thresholds (shown on plots) ----
core_padj_thr <- 0.05   # DE padj for volcano/MA and ORA seed
core_lfc_thr  <- 1.0
...
# "Standard" gene symbol detector for plotting only
is_standard_symbol <- function(x){
  x <- as.character(x)
  orf <- grepl("^Y[A-Z]{2}[0-9]{3}[CW](?:-[A-Z0-9]+)?$", x)               # ORF-like
  sym <- grepl("^[A-Z0-9][A-Z0-9-]{1,8}$", x)                              # SYMBOL-like
  orf | sym
}

# Prefer: alias_symbol -> ORF -> cleaned final_label (without sch_), but only if "standard"
...
treatment <- sub("^T[0-9]+R[0-9]+([A-Za-z])$", "\\1", sample_names)  # p / L
...
# Exclude trailing "-D" samples entirely
is_D <- grepl("D$", colnames(counts))
counts <- counts[, !is_D]; samples <- samples[!is_D, ]

# Filter genes, drop zero-sum samples if any
...
# ---- Annotation: GTF (mask duplicate gene_name) + BLASTX (dedupe/resolve) ----
gtf_raw <- readr::read_tsv(gtf_file, comment="#", col_names=FALSE, show_col_types=FALSE)
gtf_df <- gtf_raw %>% dplyr::filter(X3=="gene") %>%
  dplyr::mutate(
    raw_id   = stringr::str_extract(X9,'(?<=gene_id \")[^\"]+'),
    gtf_name = stringr::str_extract(X9,'(?<=gene_name \")[^\"]+')
  ) %>% dplyr::select(raw_id, gtf_name) %>% dplyr::distinct()

# NEW: mask any duplicated gtf_name to avoid repeated labels (e.g., SNQ2, DAG7)
dup_gtf_names <- gtf_df %>%
  dplyr::filter(!is.na(gtf_name)) %>%
  dplyr::count(gtf_name) %>%
  dplyr::filter(n > 1) %>% dplyr::pull(gtf_name)

gtf_df <- gtf_df %>%
  dplyr::mutate(gtf_name = dplyr::if_else(gtf_name %in% dup_gtf_names, NA_character_, gtf_name))

blastx_raw <- readr::read_tsv(blastx_file, show_col_types = FALSE)
blastx_df <- blastx_raw %>%
  dplyr::transmute(
    gene_id = sch_gene,
    orf_id  = dplyr::if_else(
      stringr::str_detect(sseqid, "^Y[A-Z]{2}[0-9]{3}[CW](?:-[A-Z0-9]+)?$"),
      sseqid, NA_character_
    ),
    blast_sym = dplyr::if_else(
      stringr::str_detect(stitle, "gene_symbol:[^; ]+"),
      stringr::str_extract(stitle, "(?<=gene_symbol:)[^; ]+"),
      NA_character_
    )
  ) %>% dplyr::distinct(gene_id, orf_id, blast_sym)

gene_df <- tibble::tibble(gene_id = rownames(dds))
annot0 <- gene_df %>% dplyr::left_join(gtf_df, by = c("gene_id" = "raw_id")) %>%
  dplyr::left_join(blastx_df, by = "gene_id")

# BLAST alias cleaning: drop aliases that collide with (unique) GTF names or are duplicated
gtf_used <- unique(stats::na.omit(annot0$gtf_name))
dup_blast_sym <- annot0 %>% dplyr::filter(!is.na(blast_sym)) %>%
  dplyr::count(blast_sym, name = "n_genes") %>% dplyr::filter(n_genes > 1) %>% dplyr::pull(blast_sym)
...
conflict_log <- dplyr::bind_rows(
  annot0 %>% dplyr::filter(!is.na(blast_sym), blast_sym %in% blast_conflicts_gtf) %>%
    dplyr::transmute(gene_id, conflict_type = "blast_vs_GTF_name_or_duplicate", conflicted_label = blast_sym),
...
run_go <- function(res_df){
  ...
  entrez <- stats::na.omit(entrez_map$ENTREZID); if (!length(entrez)) return(NULL)
  ...
  go_df$geneNames <- sapply(strsplit(go_df$geneID,"/"), function(ids){
    ...
    out <- ifelse(!is.na(syms) & nzchar(syms), syms, orfs)
    paste(out, collapse="/")
  })
...
run_go_clusters <- function(go_result, contrast_name, entrez_map){
  ...
  df <- go_df2 %>% dplyr::mutate(GeneRatio_num=ratio_num(GeneRatio),
                                 BgRatio_num=ratio_num(BgRatio),
                                 FoldEnrichment=GeneRatio_num/BgRatio_num,
                                 negLog10Padj=-log10(p.adjust+1e-300))
  ...
  print(
    ggraph(g, layout="fr") +
      ...
      scale_color_manual(name = "Regulation",
                         values = c("Down"="royalblue", "Up"="firebrick2"),
                         breaks = c("Down","Up"), drop = TRUE, na.translate = FALSE) +
      ...
      labs(subtitle=sprintf("Nodes: terms & significant genes. Filter: padj<%.2g & |log2FC|≥%.2f.",
...
# ---- Term–term projection by shared DE genes ----
plot_go_term_projection <- function(cluster_go_gene_csv, contrast_name,
  ...
  verts <- dplyr::bind_rows(data.frame(name=terms, type=TRUE),
                            data.frame(name=genes, type=FALSE))
...
generate_cluster_go_gene_table <- function(
  ...
  for (i in seq_len(nrow(top_clusters))) {
    ...
    go_terms <- go_df %>% dplyr::filter(ID == cluster_id); if (!nrow(go_terms)) next
...
for(name in names(contrast_list)){
  ...
  # Volcano (labels use unique plot_label; sch_ removed)
  p_vol <- EnhancedVolcano(
    res, lab=res$plot_label, x='log2FoldChange', y='padj',
    title=paste("Volcano —", species, ":", name),
    subtitle=sprintf("Thresholds: padj<%.2g, |log2FC|≥%.2f.", core_padj_thr, core_lfc_thr),
  ...
  # Heatmap (top 50 DE) — labels use unique plot_label
  ...
  res_ma <- res %>% dplyr::mutate(baseMean=ifelse(is.na(baseMean),0,baseMean)) %>% dplyr::filter(baseMean>0) %>%
    dplyr::mutate(DE=abs(log2FoldChange)>=core_lfc_thr & padj<core_padj_thr, col=ifelse(DE,"#ff7f0e","grey80"))
  with(res_ma, plot(baseMean,log2FoldChange,log='x',col=col,pch=20,cex=0.6,
                    main=paste("MA —", species, ":", name)))
  mtext(sprintf("DE highlight: padj<%.2g & |log2FC|≥%.2f.", core_padj_thr, core_lfc_thr),
  ...
  # GSEA (BP)
  entrez_map_all <- AnnotationDbi::select(org.Sc.sgd.db, keys=unique(na.omit(res$orf_id)),
                                          keytype="ORF", columns="ENTREZID")
  geneList_df <- res %>% dplyr::filter(!is.na(orf_id)) %>%
    dplyr::left_join(entrez_map_all, by=c("orf_id"="ORF")) %>%
    dplyr::filter(!is.na(ENTREZID), !is.na(log2FoldChange)) %>%
    dplyr::group_by(ENTREZID) %>% dplyr::summarise(score=mean(log2FoldChange), .groups="drop") %>% dplyr::arrange(dplyr::desc(score))
  ...
  if (!is.null(gsea_sch) && nrow(as.data.frame(gsea_sch))>0){
    ...
    p_gsea <- ggplot(topN, aes(x=NES, y=reorder(Description, NES), size=setSize, color=negLog10Padj)) +
      ...
      labs(title=paste0("GSEA (BP) — ", species, ": ", name),
           subtitle=sprintf("Points shown if FDR<%.2g. Color = -log10(FDR).", gsea_fdr_thr),
...
compare_volcano <- function(contrast_A, contrast_B, padj_thr=padj_thr_vc, lfc_thr=lfc_thr_vc, label_top=20){
  ...
  cols_named <- c(
    "New Up"="firebrick2","New Down"="royalblue",
  ...
  lab_df <- P_plot %>% dplyr::filter(status %in% c("New Up","New Down")) %>% dplyr::arrange(padj_B) %>% head_n(label_top)

  pdf(file.path(plots_dir, sprintf("VolcanoCompare_%s__minus__%s.pdf", contrast_B, contrast_A)), width=7.5, height=6.2)
  p <- ggplot(P_plot, aes(x=LFC_B, y=nlog10_p_B, color=status)) +
    ...
    ggrepel::geom_text_repel(data=lab_df, aes(label=gene_key), size=3, box.padding=0.25, max.overlaps=25) +
...
# ---- Define comparison pairs (B minus A) ----
volcano_pairs <- list(
...
message("Running volcano comparisons for ", length(volcano_pairs), " pairs...")
compare_csv_paths <- character(0)
for (pr in volcano_pairs){
  A <- pr[1]; B <- pr[2]
  out <- compare_volcano(contrast_A=A, contrast_B=B, padj_thr=padj_thr_vc, lfc_thr=lfc_thr_vc, label_top=20)
...
# ============================
# 6 category heatmaps across ALL comparisons (colored by degree; excludes non-standard symbols)
# ============================
make_category_heatmaps_all_comparisons <- function(compare_csv_paths, max_genes_per_heatmap=Inf){
  ...
  build_long_for_category <- function(cat_name, info){
    ...
    for (fp in compare_csv_paths){
      ...
      if (!nrow(D)) next
      D <- D %>% dplyr::filter(status==cat_name) %>% dplyr::filter(is_standard_symbol(gene_key))
      if (!nrow(D)) next
      if (info$fill_from=="B"){
  ...
  for (cat in names(categories)){
    ...
    M <- L %>% dplyr::filter(gene_key %in% ord_genes) %>%
      dplyr::mutate(gene_key=factor(gene_key, levels=rev(ord_genes)),
    ...
    p <- ggplot(M, aes(x=Comparison, y=gene_key, fill=value)) +
      ...
      labs(title=paste0(cat, " — ", species),
           subtitle=sprintf("Strict thresholds for categorization: padj<%.2g & |log2FC|≥%.2f.\nRows ordered by total intensity across comparisons (descending).",
                            padj_thr_vc, lfc_thr_vc),
